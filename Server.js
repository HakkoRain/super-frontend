...
const server = http.createServer(function (request, Response) { //Встроенная команда CreateServer модуля Http, для создания сервера. Принимает функцию обработчика, которая принимает 2 аргумента.
    ...
    if (request.url == '/style.css') { 
        const css = fs.readFileSync('style.css', 'utf8'); //Если происходит запрос к адресу Url, то отдаем по запросу файл со стилями style.css
    ...
    const html = fs.readFileSync('index.html', 'utf8');//Встроенная команда readFileSync модуля Fs, для четения файла index.html в кодировке utf8
    Response.end(html); //Отправляем содержимое файла index.html в ответ на запрос
    }
});

console.log('port = ', process.env.PORT); //Прием порта
